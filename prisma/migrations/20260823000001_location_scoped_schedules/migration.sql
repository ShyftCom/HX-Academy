-- Location-scoped schedules.
--
-- Before: a schedule row belonged to a *programme* (programme_schedules.programmeId,
-- NOT NULL) and only *mentioned* a location through a nullable venueId. There was one
-- effective timetable, edited from inside the programme editor.
--
-- After: a slot belongs to a *location* (stationId, NOT NULL, ON DELETE CASCADE) and the
-- programme link is optional metadata, so a location can run a session that is not part of
-- any marketed programme. Each station also gets a location_schedules header row, created
-- here for existing stations and by the API/seed for new ones.
--
-- Hand-authored to match the style of the existing migrations. As those files note, the
-- operative deploy mechanism for this project is `prisma db push` (see package.json), which
-- applies the end state described by schema.prisma. Everything here is therefore written to
-- be idempotent so it can be replayed over an already-pushed database, and the two things
-- db push cannot express — the CHECK and the EXCLUDE constraint — are also applied by
-- ensureScheduleConstraints() in prisma/schedule-constraints.ts, which the seed runs on
-- every deploy. Reversal: see down.sql in this directory.
