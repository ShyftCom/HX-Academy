  ...
  return (
    <section className="bg-white py-[var(--fsa-section-y)]">
      <div className="mx-auto max-w-3xl px-[var(--fsa-container-pad)]">
        {heading && (
          <h2 className="mb-6 font-fsa-display text-3xl font-bold uppercase tracking-tight text-fsa-navy-900 sm:text-4xl" dir="auto">{heading}</h2>
        )}
        {html && <div className="fsa-richtext text-lg text-fsa-text" dir="auto" dangerouslySetInnerHTML={{ __html: html }} />}
      </div>
    </section>
  );
}
