...
import { Trophy, Users, Zap, Shield, Star, TrendingUp, Play, ChevronDown, Menu, X, ArrowRight, Check } from "lucide-react";
...
export default function PublicLandingPage() {
  ...
  const academyName = data?.academyName ?? "HX Academy";
    ...
    <div className="min-h-screen" style={{ scrollBehavior: "smooth" } as React.CSSProperties}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-green-600 rounded-lg flex items-center justify-center font-bold text-white text-sm">HX</div>
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <a href="#plans" className="text-gray-300 hover:text-white text-sm transition-colors">Plans</a>
              <a href="#apply" className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg text-sm font-medium transition-colors">Apply Now</a>
            </div>
          ...
          {navOpen && (
            <div className="md:hidden pb-4 space-y-1 border-t border-white/10 pt-3">
              <a href="#about" className="block text-gray-300 py-2 text-sm" onClick={() => setNavOpen(false)}>About</a>
              <a href="#vsl" className="block text-gray-300 py-2 text-sm" onClick={() => setNavOpen(false)}>Video</a>
              <a href="#plans" className="block text-gray-300 py-2 text-sm" onClick={() => setNavOpen(false)}>Plans</a>
              <a href="#apply" className="block bg-green-600 text-white px-4 py-2 rounded-lg text-center text-sm mt-2 font-medium" onClick={() => setNavOpen(false)}>Apply Now</a>
            </div>
          )}
        </div>
      </nav>
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto pt-16">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-xl text-gray-300 mb-10 max-w-2xl mx-auto leading-relaxed">
              {s.lp_hero_subtitle || "Join HX Academy and unlock your football potential with world-class coaching"}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="#apply" className="inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-xl font-semibold text-lg transition-all hover:scale-105 shadow-lg shadow-green-900/40">
                {s.lp_hero_cta || "Apply Now"} <ArrowRight className="w-5 h-5" />
              </a>
      ...
      <section className="bg-green-700 py-10">
        <div className="max-w-5xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center text-white">
            {[{ num: "200+", label: "Players Trained" }, { num: "5+", label: "Years of Excellence" }, { num: "15", label: "Expert Coaches" }, { num: "98%", label: "Success Rate" }].map((stat, i) => (
              <div key={i}>
      ...
      <section id="about" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <div className="inline-flex items-center gap-2 bg-green-100 text-green-700 rounded-full px-4 py-1.5 text-sm font-semibold mb-6">About The Academy</div>
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6 leading-tight">
                {s.lp_about_title || "Building Champions On and Off the Field"}
              </h2>
              <p className="text-gray-600 leading-relaxed mb-8 text-lg">
                {s.lp_about_content || "HX Academy was founded with a single mission: to develop complete footballers. We combine elite technical training with personal development, creating players who excel both on the pitch and in life."}
              </p>
              <div className="space-y-3">
                {["Professional UEFA-certified coaching staff", "Individual player development plans", "Regular performance assessments"].map((pt, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center flex-shrink-0"><Check className="w-3.5 h-3.5 text-green-600" /></div>
      ...
      <section id="vsl" className="py-20 bg-gray-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-green-600/20 border border-green-500/30 rounded-full px-4 py-1.5 text-green-400 text-sm font-medium mb-6">In Action</div>
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{s.lp_vsl_title || "See What We Do"}</h2>
          <p className="text-gray-400 mb-12 max-w-xl mx-auto">Watch our players train, compete, and grow with the academy.</p>
            ...
            {s.lp_vsl_url ? (
              ...
              <div className="relative w-full rounded-2xl overflow-hidden bg-gray-800 border border-gray-700 flex items-center justify-center" style={{ minHeight: "360px" }}>
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-sm">Video coming soon — configure in admin dashboard</p>
                </div>
              </div>
            )}
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <div className="inline-flex items-center gap-2 bg-green-100 text-green-700 rounded-full px-4 py-1.5 text-sm font-semibold mb-4">Why Us</div>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Why Choose HX Academy</h2>
          </div>
      ...
      <section id="plans" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <div className="inline-flex items-center gap-2 bg-green-100 text-green-700 rounded-full px-4 py-1.5 text-sm font-semibold mb-4">Pricing</div>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Choose Your Plan</h2>
            <p className="text-gray-500 mt-3">Flexible plans designed for every level of commitment</p>
          </div>
          {plans.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {plans.map((plan: any, i: number) => (
                <div key={plan.id} className={`relative rounded-2xl border-2 p-6 flex flex-col transition-all hover:shadow-lg ${i === 0 ? "border-green-500 shadow-md" : "border-gray-200"}`}>
                  {i === 0 && <div className="absolute -top-3 left-1/2 -translate-x-1/2"><span className="bg-green-600 text-white text-xs font-bold px-3 py-1 rounded-full">Most Popular</span></div>}
                  <div className="h-1 rounded-full mb-5" style={{ backgroundColor: plan.color || "#16a34a" }} />
                  ...
                  {plan.description && (
                    <ul className="space-y-2 mb-6 flex-1">
                      {plan.description.split("\n").filter(Boolean).map((line: string, j: number) => (
                        <li key={j} className="flex items-start gap-2 text-sm text-gray-600"><Check className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />{line.replace(/^[-•]\s*/, "")}</li>
                      ))}
                    </ul>
                  )}
                  <Link href={`/apply?plan=${plan.id}`} className={`mt-auto block text-center py-3 rounded-xl font-semibold transition-all ${i === 0 ? "bg-green-600 hover:bg-green-700 text-white" : "border-2 border-green-600 text-green-600 hover:bg-green-50"}`}>
                    Apply Now
                  </Link>
                </div>
              ))}
      ...
      <section id="apply" className="py-20 bg-gradient-to-br from-green-700 to-green-900">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Ready to Join the Academy?</h2>
          <p className="text-green-200 mb-10 text-lg">Start your application today and take the first step toward becoming a champion.</p>
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-8 max-w-2xl mx-auto text-left mb-8">
            <p className="text-white font-semibold mb-5 text-center">Complete your application in 4 simple steps:</p>
            <div className="space-y-3">
              {["Choose your subscription plan", "Fill in your personal information", "Upload required documents", "Submit and wait for our team to contact you"].map((step, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="w-8 h-8 bg-green-600 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">{i + 1}</div>
      ...
      <footer className="bg-gray-950 py-12">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-3 mb-3">
            <div className="w-10 h-10 bg-green-600 rounded-lg flex items-center justify-center font-bold text-white">HX</div>
            <span className="text-white font-bold text-xl">{academyName}</span>
          </div>
          <p className="text-gray-500 text-sm mb-6">Building Champions Since 2019</p>
          <div className="border-t border-gray-800 pt-6">
            <p className="text-gray-600 text-sm">{s.lp_footer_text || `© ${new Date().getFullYear()} ${academyName}. All rights reserved.`}</p>
          </div>
        </div>
      </footer>
    </div>
