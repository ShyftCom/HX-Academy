...
export default function PipelinePage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["lead-statuses"] }); toast.success(t("pipeline.updated")); setEditingId(null); },
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["lead-statuses"] }); toast.success(t("pipeline.created")); setShowNewForm(false); setNewName(""); setNewColor("#6B7280"); },
    onError: () => toast.error(t("common:toast.create_failed")),
  });
  ...
  const reorderMutation = useMutation({
    ...
    onError: () => toast.error(t("pipeline.reorder_failed")),
  });
  ...
  function saveEdit(s: LeadStatus) {
    if (!editName.trim()) return toast.error(t("pipeline.name_required"));
    saveMutation.mutate({ id: s.id, body: { name: editName.trim(), color: editColor } });
  }

  function handleCreate() {
    if (!newName.trim()) return toast.error(t("pipeline.name_required"));
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm" style={{ color: "var(--text-muted)" }}>
        <Link href="/dashboard/leads" className="hover:underline">{t("common:ui.leads")}</Link>
        <ChevronRight className="w-3.5 h-3.5" />
        <span style={{ color: "var(--text-primary)" }}>{t("pipeline.title")}</span>
      </div>
            ...
            {editingId === s.id ? (
              <div className="p-4 space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    placeholder={t("pipeline.stage_name")}
                  />
              ...
              <div className="flex items-center gap-3 p-3">
                ...
                <div className="flex items-center gap-1 ms-1 flex-shrink-0">
                  {s.isTerminal && (
                    <span className="text-xs px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--muted-bg)", color: "var(--text-muted)" }}>{t("pipeline.terminal")}</span>
                  )}
                  {s.isDefault && (
                    <span className="text-xs px-1.5 py-0.5 rounded font-medium" style={{ background: "var(--muted-bg)", color: "var(--text-muted)" }}>{t("common:ui.default")}</span>
                  )}
                </div>
                <div className="flex items-center gap-1 ms-auto flex-shrink-0">
                  <button onClick={() => startEdit(s)} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--text-muted)" }}
                    title={t("common:ui.edit")}>
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  {s.isDefault ? (
                    <button className="p-1.5 rounded-lg opacity-30 cursor-not-allowed" title={t("pipeline.cannot_delete")}>
                    ...
                    <button onClick={() => { setDeleteTarget(s); setFallbackId(statuses.find((x) => x.id !== s.id)?.id ?? ""); }}
                      className="p-1.5 rounded-lg transition-colors" style={{ color: "#A02020" }} title={t("common:ui.delete")}>
      ...
      {showNewForm ? (
        <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--card)", border: "1px solid var(--card-border)" }}>
          <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{t("pipeline.new_stage")}</p>
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="flex-1 rounded-lg px-3 py-2 text-sm outline-none"
              style={{ background: "var(--input-bg)", border: "1px solid var(--input-border)", color: "var(--text-primary)" }}
              placeholder={t("pipeline.stage_name_max")}
            />
          </div>
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowNewForm(false)} className="px-3 py-1.5 text-sm rounded-lg" style={{ border: "1px solid var(--card-border)", color: "var(--text-muted)" }}>{t("common:ui.cancel")}</button>
        ...
        <button onClick={() => setShowNewForm(true)}
          ...
          <Plus className="w-4 h-4" /> {t("pipeline.new_stage")}
        </button>
      )}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="rounded-2xl shadow-2xl p-6 max-w-md w-full space-y-4" style={{ background: "var(--card)", border: "1px solid var(--card-border)" }}>
            ...
            <div>
              <label className="text-sm font-medium block mb-1.5" style={{ color: "var(--text-secondary)" }}>{t("pipeline.move_to")}</label>
              <select
                value={fallbackId}
                ...
                style={{ background: "var(--input-bg)", border: "1px solid var(--input-border)", color: "var(--text-primary)" }}
              >
                <option value="">{t("pipeline.no_status")}</option>
                {otherStatuses.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>

            <div className="flex justify-end gap-2 pt-1">
              <button onClick={() => setDeleteTarget(null)} className="px-4 py-2 text-sm rounded-lg" style={{ border: "1px solid var(--card-border)", color: "var(--text-muted)" }}>{t("common:ui.cancel")}</button>
              <button
