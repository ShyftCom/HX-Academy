...
export default function LeadsPage() {
  ...
  const columns = [
    ...
    { key: "status", header: "Status", cell: (r: any) => r.status ? (
      <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium" style={{ backgroundColor: r.status.color + "20", color: r.status.color }}>
        {r.status.name}
      </span>
    ) : "—" },
        ...
        <DialogContent size="2xl">
          ...
          <form onSubmit={handleSubmit((d) => saveMutation.mutate(d))}>
            <DialogBody className="grid grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Category Interest</label>
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Assign to Staff</label>
