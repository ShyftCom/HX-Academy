    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="me-2 h-4 w-4" />{t("staff.add")}</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>{t("staff.add_title")}</DialogTitle></DialogHeader>
            <div className="space-y-3 py-2">
              <div className="space-y-1">
                <Label>{t("staff.user_account")}</Label>
                <select className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm" value={form.userId} onChange={(e) => setForm((f) => ({ ...f, userId: e.target.value }))}>
                  <option value="">{t("staff.select_user")}</option>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><Label>{t("staff.role")}</Label><Input value={form.role} onChange={(e) => setForm((f) => ({ ...f, role: e.target.value }))} placeholder={t("staff.role_ph")} /></div>
                ...
                <div className="space-y-1"><Label>{t("staff.base_salary_da")}</Label><Input type="number" value={form.baseSalary} onChange={(e) => setForm((f) => ({ ...f, baseSalary: e.target.value }))} /></div>
              </div>
