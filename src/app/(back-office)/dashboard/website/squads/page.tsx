...
interface SquadLead {
  ...
  parentName: string | null; categoryInterest: string | null; notes: string | null;
  extraData: string | null; status: LeadStatus | null; createdAt: string;
}
...
export default function SquadsSubmissionsPage() {
  ...
  const { data, isLoading } = useQuery<{ data: SquadLead[]; total: number }>({
    queryKey: ["admin-squad-leads"],
    queryFn: () => fetch("/api/applications?leadType=squad_registration&perPage=100").then((r) => r.json()),
  });
  ...
  const { mutate: updateStatus, isPending: updating } = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin-squad-leads"] }),
  });
              ...
              <div key={lead.id} className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
                <button onClick={() => setExpandedId(expanded ? null : lead.id)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-start hover:bg-gray-50 dark:hover:bg-gray-800/60">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Parent: {lead.parentName} · {new Date(lead.createdAt).toLocaleDateString()}</p>
                  </div>
                  <select
